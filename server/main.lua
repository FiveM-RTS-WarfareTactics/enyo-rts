...
-- Exports
exports('GetMaps', function() return Config.Maps end)
exports('GetMapData', function(name) return Config.Maps and Config.Maps[name] end)
exports('GetUnits', function() return Config.Units end)
exports('GetUnitData', function(key) return Config.Units and Config.Units[key] end)
exports('GetUnitCategories', function() return Config.UnitCategories end)
...
-- Chat
RegisterNetEvent('rts:server:sendChatMessage', function(message)
    local src = source
    if GetResourceState('rts-admin') == 'started' and exports['rts-admin']:IsPlayerMuted(src) then
        TriggerClientEvent('rts:client:receiveChatMessage', src, 'SYSTEM', 'You are muted.', 'system')
        return
    end
    local matchId, match = GetPlayerMatch(src)
    local playerName = GetPlayerName(src)

    if PlayerStates and PlayerStates[src] and PlayerStates[src].playerName then
        playerName = PlayerStates[src].playerName
    end

    if not matchId and Lobbies then
        for code, lobbyData in pairs(Lobbies) do
            for _, pid in ipairs(lobbyData.players) do
                if pid == src then
                    matchId = code
                    match = lobbyData
                    break
    ...
    if matchId and match then
        local channelType = (match.active and "match") or "lobby"

        if match.active then
            for pid, _ in pairs(match.players) do
                if type(pid) == "number" then
                    TriggerClientEvent('rts:client:receiveChatMessage', pid, playerName, message, channelType)
                end
            end
        else
        ...
        local status = (match.active and "MATCH") or "LOBBY"
        local logMsg = string.format("🎮 **[%s #%s]**\n👤 **%s**:\n💬 %s", status, matchId, playerName, message)
        SendDiscordLog(Webhooks.Chat, "RTS Room Chat", logMsg, 15158332)
    else
        local players = GetPlayers()
        for _, pidStr in ipairs(players) do
            local pid = tonumber(pidStr)
            local mId, m = GetPlayerMatch(pid)
            if not mId then
                TriggerClientEvent('rts:client:receiveChatMessage', pid, playerName, message, "global")
            end
        end

        local logMsg = string.format("🌐 **[GLOBAL LOBBY]**\n👤 **%s**:\n💬 %s", playerName, message)
        SendDiscordLog(Webhooks.Chat, "RTS Global Chat", logMsg, 3447003)
    end
end)