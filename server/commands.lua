...
RegisterNetEvent('enyo-rts:server:adminForceEnd', function(matchId)
    local src = source
    if matchId == 'all' then
        for id, _ in pairs(Matches or {}) do
            EndMatch(id, { type = "admin_terminated", winner = 0 })
        end
        print(string.format("^1[RTS ADMIN] ALL matches forcibly terminated by Admin ID: %s^7", src))
    elseif Matches[matchId] then
